...
use App\Models\Auth\TwoFactorAuthentication;
use App\Events\TwoFactor\TwoFactorEnabled;
use App\Events\TwoFactor\TwoFactorDisabled;
use App\Events\TwoFactor\TwoFactorRecoveryCodesGenerated;
use App\Events\TwoFactor\TwoFactorRecoveryCodesDeleted;
...
trait TwoFactorAuthenticator
{
    ...
    {
        return $this->morphOne(TwoFactorAuthentication::class, 'authenticatable')->withDefault(config('auth2fa.totp'));
    }
    ...
    public function enableTwoFactorAuth(): void
    {
        ...
        if(config('auth2fa.recovery.enabled')) {
     ...
     *
     * @return \App\Contracts\Auth\TwoFactorTotp
     */
    ...
    public function generateRecoveryCodes(): Collection
    {
        [$amount, $length] = array_values(config()->get([
            'auth2fa.recovery.codes', 'auth2fa.recovery.length'
        ]));
    ...
    protected function useRecoveryCode(string $code): bool
    {
        ...
        if (!$this->hasRecoveryCodes()) {
            event(new TwoFactorRecoveryCodesDeleted($this));
        }
    ...
    public function addSafeDevice(Request $request): string
    {
        [$name, $expiration] = array_values(config()->get([
            'auth2fa.safe_devices.cookie', 'auth2fa.safe_devices.expiration_days'
        ]));
        ...
        $this->twoFactorAuth->safe_devices = $this->safeDevices()
            ...
            ->slice(0, config('auth2fa.safe_devices.max_devices', 3))
            ->values();
    ...
    {
        return TwoFactorAuthentication::generateDefaultTwoFactorRemember();
    }
    ...
    public function isSafeDevice(Request $request): bool
    {
        ...
        if ($timestamp) {
            return $timestamp->addDays(config('auth2fa.safe_devices.expiration_days'))->isFuture();
        }
    ...
    {
        return $request->cookie(config('auth2fa.safe_devices.cookie', '2fa_remember'));
    }
