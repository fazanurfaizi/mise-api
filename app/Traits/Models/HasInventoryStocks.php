...
trait HasInventoryStocks
{
    ...
    /**
     * Stores the quantity after an update.
     *
     * @var int|float|string
     */
    private $afterQuantity = 0;
    ...
    private function processTakeOperation($taking, $reason = '', $cost = 0)
    {
        try {
            ...
            */
            ...
            $this->fireModelEvent('stock.taken', [
                'stock' => $this,
            ]);
    ...
    private function processMoveOperation(Model $warehouse)
    {
        ...
        try {
            if ($this->save()) {
                ...
                $this->fireModelEvent('stock.moved', [
                    'stock' => $this,
                ]);
    ...
    private function processRollbackOperation(Model $movement, $recursive = false)
    {
        ...
        try {
            ...
            if ($this->save()) {
                ...
                $this->fireModelEvent('stock.rollback', [
                    'stock' => $this,
                ]);
